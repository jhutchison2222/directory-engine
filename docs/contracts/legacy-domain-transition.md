...
- `reference` is a required, typed, attributable evidence reference:
  ...
  - Neither `recorded_by` nor `citation` may embed credential or secret
    material. This is checked structurally, not just against two examples:
    any URL with non-empty userinfo before `@` (`user:pass@host` or
    `token@host`, with or without a colon), a credential/secret assignment
    (for example `password=`, `token:`, `api_key=`), or a common vendor
    token/access-key shape (for example a GitHub `ghp_…` token, an
    OpenAI/Anthropic-style `sk-…` key, an AWS `AKIA…` access key ID, a Slack
    `xox…-…` token, a `bearer …` header, an `authorization:` header, or a
    `-----BEGIN` PEM block). Any match is an **evidence-reference-credential**
    violation, since an evidence citation must never carry a live credential.
