...
const INVALID_FIXTURES = [
  ...
  ["de-0009-invalid-evidence-reference-credential-token-userinfo.json", "evidence-reference-credential"],
  ["de-0009-invalid-evidence-reference-credential-token-shape-recorded-by.json", "evidence-reference-credential"],
  ["de-0009-invalid-evidence-reference-credential-token-shape-citation.json", "evidence-reference-credential"],
...
describe("evidence-reference structure", () => {
  ...
  it("rejects a blank citation as missing attribution", async () => {
    const fixture = await loadFixture("de-0009-invalid-blank-citation.json");
    const errors = validateLegacyDomainTransition(fixture, registryFixture);
    expect(errors.some((message) => message.startsWith("missing-evidence-attribution:"))).toBe(true);
  });

  it("rejects a reference_type outside the recognized enum", async () => {
  ...
  it("rejects token-only URL userinfo (no colon) as embedded credential material", async () => {
    const fixture = await loadFixture("de-0009-invalid-evidence-reference-credential-token-userinfo.json");
    const errors = validateLegacyDomainTransition(fixture, registryFixture);
    expect(errors.some((message) => message.startsWith("evidence-reference-credential:"))).toBe(true);
  });

  it("rejects a raw token/access-key-shaped value in recorded_by", async () => {
    const fixture = await loadFixture("de-0009-invalid-evidence-reference-credential-token-shape-recorded-by.json");
    const errors = validateLegacyDomainTransition(fixture, registryFixture);
    expect(errors.some((message) => message.startsWith("evidence-reference-credential:"))).toBe(true);
  });

  it("rejects a raw token/access-key-shaped value in citation", async () => {
    const fixture = await loadFixture("de-0009-invalid-evidence-reference-credential-token-shape-citation.json");
    const errors = validateLegacyDomainTransition(fixture, registryFixture);
    expect(errors.some((message) => message.startsWith("evidence-reference-credential:"))).toBe(true);
  });
});
