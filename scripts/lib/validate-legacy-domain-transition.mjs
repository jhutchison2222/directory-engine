...
// a live credential: a URL with any non-empty userinfo before "@" (with or
// without a colon), a credential/secret assignment (`token=...`,
// `api_key: ...`), a common vendor token/access-key shape (GitHub `ghp_...`,
// OpenAI/Anthropic-style `sk-...`, AWS `AKIA...`, Slack `xox?-...`), or a
// bearer/authorization header. Intentionally broad and fails closed on any
// match; every alternative below is a structural shape detector, not a real
// credential.
const CREDENTIAL_URL_PATTERN = /:\/\/[^\s/@]+@/;
const CREDENTIAL_KEYWORD_PATTERN =
  /(password|passwd|secret|token|api[_-]?key|access[_-]?key|private[_-]?key|client[_-]?secret|bearer\s|authorization\s*:|-----begin|\bgh[oprsu]?_[a-z0-9]{10,}\b|\bgithub_pat_[a-z0-9_]{10,}\b|\bsk-[a-z0-9-]{10,}\b|\bxox[baprs]-[a-z0-9-]{6,}\b|\bakia[0-9a-z]{10,}\b)/i;
